...
const OPACITY_COLORS = [
  { token: '--color-white-subtle',  label: 'White Subtle',  usage: 'Sidebar active nav bg; topbar search input bg (navy context)' },
  { token: '--color-white-dim',     label: 'White Dim',     usage: 'Topbar search border' },
  ...
  { token: '--color-sand-label',    label: 'Sand Label',    usage: 'Sidebar section header labels (MY SPACE etc.)' },
  { token: '--color-sand-muted',    label: 'Sand Muted',    usage: 'Sidebar inactive nav items, Sign Out button' },
  { token: '--color-navy-backdrop', label: 'Navy Backdrop', usage: 'Mobile sidebar overlay scrim (Sidebar.tsx)' },
];

const ROW_COLORS = [
  { token: '--color-row-divider', label: 'Row Divider', usage: 'border-bottom on every compliment row' },
  { token: '--color-row-hover',   label: 'Row Hover',   usage: 'Hover bg on rows, find-fragrance results, skeleton' },
  { token: '--color-meta-text',   label: 'Meta Text',   usage: 'Relation · gender · location meta; compliment notes; input borders' },
];
...
const TYPE_TOKENS = [
  ...
  { token: '--text-page-title', label: 'Page Title',  role: 'Topbar page title (Compliments)',             font: 'serif', italic: true  },
  { token: '--text-logo',       label: 'Logo',        role: 'Sidebar tęsknota logotype',                  font: 'serif', italic: true  },
];

const SPACE_TOKENS = [
  '--space-1', '--space-4', '--space-5', '--space-6', '--space-8', '--space-10',
];
            ...
            <div style={{ marginTop: 'var(--space-4)' }}>
              {TYPE_TOKENS.map(({ token, label, role, font, italic }) => (
                <div
                  ...
                  style={{ borderBottom: '1px solid var(--color-row-divider)', padding: 'var(--space-2) 0' }}
                >
                  <span
